...
- 修复 0.10.0 五子棋和井字棋首步写入 500：移除生产环境不兼容的条件批量更新，改用应用其他模块已长期验证的单文档写入，并保留 revision 二次校验和动作幂等
- 游戏发布健康检查不再只读集合，而会真实写入、读取、验证并清理一手测试棋，避免“流水线通过但真机不能落子”
- 新增“情侣杯”累计比分：三款游戏分别记录胜场、平局、总局数和连胜，并在游戏屋汇总两人的总比分
- 五子棋、井字棋每局自动交换先后手；胜负、认输和平局都会正确计分，结果页增加庆祝反馈和手机轻震
- 取消开局、每次落子和每次出拳产生的自动通知；整局结果只进入小程序消息盒子，不消耗微信订阅额度
- 新增手动“提醒来玩”：仅在确实等待对方时由用户主动发送，同一游戏 30 分钟内最多一次
- 游戏中心重新排版，加入双方角色、局内比分、连胜、结果卡片和更清晰的同步状态
...
正式上线的低成本方案与容量边界见 [`docs/hosting-cost-0.4.md`](docs/hosting-cost-0.4.md)，支付暂缓，v0.11 上线与真机验收见 [`release-assets/0.11.0-release-notes.md`](release-assets/0.11.0-release-notes.md)。
