...
const version = process.env.WECHAT_VERSION || "0.4.0";
const desc = (process.env.WECHAT_DESC || "情侣小农场 0.4.0：今天一起、餐厅抽签与会员内测").slice(0, 32);
...
if (!/^\d+\.\d+\.\d+(?:[-.][0-9A-Za-z]+)?$/.test(version)) {
  fail("WECHAT_VERSION 必须是类似 0.4.0 的版本号。");
}
