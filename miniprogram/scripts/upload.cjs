...
const version = process.env.WECHAT_VERSION || "0.5.0";
const desc = (process.env.WECHAT_DESC || "我们俩的小田地 0.5.0：共同小本本、提醒与村口修复").slice(0, 32);
...
if (!/^\d+\.\d+\.\d+(?:[-.][0-9A-Za-z]+)?$/.test(version)) {
  fail("WECHAT_VERSION 必须是类似 0.5.0 的版本号。");
}
